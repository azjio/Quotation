...
;- Lang En
Lng(1) = "Portable?"
Lng(2) = "Create a config and a working folder next to the executable file?"
Lng(3) = "Error"
Lng(4) = "There is not a single section of settings for saving quotes in the ini file."
Lng(5) = "Open the ini file?"
Lng(6) = "Could not create a file for quotes."
Lng(7) = "Could not open a file for quotes."
Lng(8) = "Couldn't create a folder for quotes."
Lng(9) = "Couldn't define and create a path:"
Lng(10) = "Folder name error"
Lng(11) = "Set folder mask correctly: "
Lng(12) = "Folder creation error"
Lng(13) = "Couldn't create folder: "
Lng(14) = "Set file name"
Lng(15) = "Empty - Cancel"
Lng(16) = "Apply"
Lng(17) = "Show settings"
Lng(18) = "Help"
Lng(19) = "Open a quote file"
Lng(20) = "Open the working folder"
Lng(21) = "Open the ini"
Lng(22) = "About"
Lng(23) = "Exit"
Lng(24) = "Preview"
Lng(25) = "Cancel"
Lng(26) = "Failed to register hotkey"
Lng(27) = "Cancel hotkey (Backspace)"
Lng(28) = "Author"

;- Lang Ru
If UserIntLang
	Lng(1) = "Портативная?"
	Lng(2) = "Создать конфиг и рабочую папку рядом с исполняемым файлом?"
	Lng(3) = "Ошибка"
	Lng(4) = "В ini-файле нет ни одной секции настроек для сохранения цитат."
	Lng(5) = "Открыть ini-файл?"
	Lng(6) = "Не удалось создать файл для цитат."
	Lng(7) = "Не удалось открыть файл для цитат."
	Lng(8) = "Не удалось создать папку для цитат."
	Lng(9) = "Не удалось определить и создать путь:"
	Lng(10) = "Ошибка имени папки"
	Lng(11) = "Задайте правильно маску папки: "
	Lng(12) = "Ошибка создания папки"
	Lng(13) = "Не удалось создать папку: "
	Lng(14) = "Задайте имя файла"
	Lng(15) = "Пустое - Отмена"
	Lng(16) = "Применить"
	Lng(17) = "Показать настройки"
	Lng(18) = "Справка"
	Lng(19) = "Открыть файл с цитатами"
	Lng(20) = "Открыть рабочую папку"
	Lng(21) = "Открыть ini"
	Lng(22) = "О программе"
	Lng(23) = "Выход"
	Lng(24) = "Предпросмотр"
	Lng(25) = "Отмена"
	Lng(26) = "Не удалось зарегистрировать горячую клавишу"
	Lng(27) = "Отмена горячей клавиши (Backspace)"
	Lng(28) = "Автор"
EndIf
...
Global ini$
If FileSize(PathConfig$ + "Quotation.ini") < 0 And MessageRequester(Lng(1), Lng(2), #PB_MessageRequester_YesNo) = #PB_MessageRequester_No
...
If Not ListSize(iniGroups())
	If MessageRequester(Lng(3), Lng(4) + #CRLF$ + #CRLF$ + Lng(5), #PB_MessageRequester_YesNo) = #PB_MessageRequester_Yes
		RunProgram(ini$)
	EndIf
	End
EndIf
...
; 					Бэкапируем если размер файла превышен
				...
				If OpenFile(#File, iniGroups()\path, iniGroups()\wstr)
					...
					CloseFile(#File)
				Else
					MessageRequester(Lng(3), Lng(7))
				EndIf
				
			Case 2
				If Not iniGroups()\isExistPath
					If iniGroups()\isExistPath = -1
						MessageRequester(Lng(3), Lng(8))
						ProcedureReturn
					EndIf
...
; 					Debug Mid(iniGroups()\path, 2, 2)
					...
					Else
; 						MessageRequester(Lng(3), Lng(8))
						MessageRequester("", Lng(9) + #CRLF$ + iniGroups()\path)
						iniGroups()\isExistPath = -1
						ProcedureReturn
					EndIf
...
; 					EndIf
				...
				If Not CheckFilename(folder$)
					MessageRequester(Lng(10), Lng(11) + iniGroups()\folder + #CRLF$ + folder$)
					ProcedureReturn
				EndIf
				...
				If FileSize(iniGroups()\path + folder$) <> -2 And Not CreateDirectory(iniGroups()\path + folder$)
					MessageRequester(Lng(12), Lng(13) + #CRLF$ + iniGroups()\path + folder$)
					iniGroups()\isExistPath = -1
					ProcedureReturn
				EndIf
				...
				Select iniGroups()\genname
					...
					Default
						Repeat
							name$ = InputRequester(Lng(14), Lng(15), "")
							If Not Asc(name$)
								ProcedureReturn
							EndIf
...
; 	EndIf
	ButtonGadget(#btnApply, 130, 40, 100, 28, Lng(16))


;- ├ Menu создание
	If CreatePopupMenu(#Menu) ; Создаёт всплывающее меню
		MenuItem(#mShow, Lng(17))
		MenuItem(#mHelp, Lng(18))
		MenuItem(#mQFile, Lng(19))
		MenuItem(#mQFolder, Lng(20))
		MenuItem(#mINI, Lng(21))
		MenuItem(#mAbout, Lng(22))
		MenuBar()
		MenuItem(#mExit, Lng(23))
	EndIf
	#SysTrayIcon = 0
...
EndIf
		...
		;-┌──GUI──┐
hWnd_1 = OpenWindow(#WindowPreview, 0, 0, ini_width, ini_height, Lng(24), #PB_Window_SystemMenu | #PB_Window_ScreenCentered | #PB_Window_MinimizeGadget | #PB_Window_MaximizeGadget | #PB_Window_SizeGadget | #PB_Window_Invisible)
...
If hWnd_1
	...
	ButtonGadget(#btnCancel, w - 220, h - 35, 100, 30, Lng(25))
...
; 					SetWindowState(#WindowSet, #PB_Window_Normal)
					HideWindow(#WindowSet, #False)
				Case #mAbout
					MessageRequester(Lng(22), Lng(28) + " AZJIO (" + #date$ + ")")
				Case #mEsc
					HideWindow(#WindowPreview, #True)
					SetGadgetText(#Editor , "")
					ProcedureReturn
